...
class Node:
    def __init__(self, env: simpy.Environment, id: int, x: float, y: float):
        ...
        self.env = env
        self.energy = 0
        self.harvest_incline_rate = random.uniform(0.0000001, 0.1) # Random incline
        self.neighbors = {}
    ...
    def run(self):
        while True:
            EnergyLogger().log(self.id, self.env.now, self.energy)
    ...
    def harvest(self, time):
        ...
        self.energy = min(self.energy + rate, E_MAX)
    ...
    def listen(self, duration):
        ...
        available_sec = math.floor(min(duration, max_sec))
